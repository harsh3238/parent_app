...
class GalleryPhotoViewer extends StatefulWidget {
  GalleryPhotoViewer({this.photo});

  final Photo photo;

  @override
  _GalleryPhotoViewerState createState() {
    return _GalleryPhotoViewerState();
  }
}
...
class _GalleryPhotoViewerState extends State<GalleryPhotoViewer>
  ...
  void takeScreenShotWhatsapp() async {
    setState(() {
      _offset = Offset.zero;
      _scale = 1.0;
    });
    ...
    screenshotController.capture().then((File image) async {
      ...
      image.readAsBytes().then((data) async {
        ...
        String codeApended = "data:image/png;base64," + base64Image;
        /*AdvancedShare.whatsapp(msg: "", url: codeApended)
            .then((response) {
        });*/
      });
    });
