...
import org.flipacademy.databinding.ActivitySubjectiveTestBinding
import org.flipacademy.interfaces.NetworkClient
import org.flipacademy.mvps.ncert.PDFViewActivity
...
class ActivitySubjectiveTestRoom : AppCompatActivity() {
    ...
    private lateinit var progressBar: LoaderDialog
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        progressBar = LoaderDialog(this, R.style.PurpleTheme)
        ...
        mAdapter.onItemClick = { pos, view, item ->
            val d = DialogPhotoViewer(this@ActivitySubjectiveTestRoom, R.style.Theme_AppCompat_NoActionBar, item)
            d.show()
        }
            ...
            Dexter.withActivity(this@ActivitySubjectiveTestRoom)
                    ...
                    .withListener(object : PermissionListener {
                        ...
                        override fun onPermissionRationaleShouldBeShown(permission: PermissionRequest?, token: PermissionToken?) {
                            val PermissionRationaleDialog = CustomAlertDialog(this@ActivitySubjectiveTestRoom, R.style.PurpleTheme)
    ...
    private fun captureAnImage() {
        val submissionsDir = File(cacheDir.absolutePath + "/submissions/submissions_${intent.getStringExtra("test_id")}")
        if (!submissionsDir.exists()) {
            submissionsDir.mkdirs()
        }

        val tempCapturedFile = File(submissionsDir.absolutePath + "/" + System.currentTimeMillis() + ".jpg")
        val imageUri = FileProvider.getUriForFile(this@ActivitySubjectiveTestRoom,
                "org.flipacademy.fileprovider", tempCapturedFile)
    ...
    private fun populateRecyclerView() {
        val filesDir = File(cacheDir.absolutePath + "/submissions/submissions_${intent.getStringExtra("test_id")}")
    ...
    private fun submitRoutine() {
        if(mUserId.isBlank()){
            Toast.makeText(this@ActivitySubjectiveTestRoom, "User not initialised, you may need to logout and login again", Toast.LENGTH_SHORT)
                    .show()
            return
        }
        ...
        if (files != null && files.isNotEmpty()) {
            ...
            Toast.makeText(this@ActivitySubjectiveTestRoom, "No Submissions found", Toast.LENGTH_SHORT)
                    .show()
        }
    }
    ...
    private fun compressAndZipFiles() {
        ...
        doAsync {
            ...
            if (files != null && files.isNotEmpty()) {
                val list = mutableListOf<String>()
                val dFile =  File(cacheDir.absolutePath + "/submissions/submissions_${intent.getStringExtra("test_id")}_comp")
                files.forEach {
    ...
    private fun getQuestionPaper() {
        if (!(application as MyApplication).isNetworkAvailable()) {
            val noInternet = DialogNoInternet(this, R.style.PurpleTheme, this::getQuestionPaper)
            noInternet.show()
            return
        }
        progressBar.show()
        ...
        val call = networkClient.getSubjectiveTest(intent.getStringExtra("test_id")!!)
        call.enqueue(object : Callback<String> {
                    ...
                    if (response.isSuccessful) {
                        ...
                        if (jsonObject.has("status") &&
                            ...
                            if (jsonArray.getString("media_type") == "image") {
                                val d = DialogPhotoViewer(this@ActivitySubjectiveTestRoom, R.style.Theme_AppCompat_NoActionBar, jsonArray.getString("file_url"))
                                d.show()
                                progressBar.dismiss()
                                ...
                                if (imageCacheDir.exists()) {
                                    val intent = Intent(this@ActivitySubjectiveTestRoom, PDFViewActivity::class.java)
                                ...
                                val downloadCall = networkClient.downloadFile(fileUrl)
                                downloadCall.enqueue(object : Callback<ResponseBody?> {
                                    override fun onFailure(call: Call<ResponseBody?>, t: Throwable) {
                                        Toast.makeText(this@ActivitySubjectiveTestRoom, "Download failed", Toast.LENGTH_SHORT)
                                                .show()
                                    }
            ...
            override fun onFailure(call: Call<String>?, t: Throwable?) {
                progressBar.dismiss()
                Toast.makeText(this@ActivitySubjectiveTestRoom, "There has been error, please try again", Toast.LENGTH_SHORT)
                        .show()
            }
                    ...
                    if (response.isSuccessful) {
                        ...
                        if (jsonObject.has("status") && jsonObject.getString("status") == "success") {
                            ...
                            upload.enqueue(object : Callback<String?> {
                                override fun onFailure(call: Call<String?>, t: Throwable) {
                                    progressBar.dismiss()
                                    Toast.makeText(this@ActivitySubjectiveTestRoom, "Not uploaded", Toast.LENGTH_SHORT)
                                            .show()
                                }
    ...
    private fun submitTests(s3FileKey: String) {
        ...
        val call = networkClient.saveSubjectiveTests((applicationContext as MyApplication).usersData.id,
            ...
            override fun onResponse(call: Call<String>?, response: Response<String>?) {
                response?.let {
                    if (response.isSuccessful) {
                        val jsonObject = JSONObject(response.body().toString())
                        if (jsonObject.has("status") && jsonObject.getString("status") == "success") {
                            Toast.makeText(this@ActivitySubjectiveTestRoom, "Test Submitted", Toast.LENGTH_SHORT)
                                    .show()
                            finish()
                        }
    ...
    private fun start() {
        ...
        countDownTimer = object : CountDownTimer(remainingTime, 1000) {
            ...
            override fun onTick(millisUntilFinished: Long) {
                ...
                mContentView.countDownTimer.text = "${format.format(hours)}:${format.format(minutes)}:${format.format(seconds)}"
